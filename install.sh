...
for file in flake.nix configuration.nix settings.nix impermanence.nix desktop.nix style.nix; do
  [[ -f $source_dir/$file ]] || die "Missing $file next to install.sh."
done
ask() {
  local value
  read -r -p "$2 [$3]: " value
...
# Resolve and lock the release before asking to erase anything.
staging=$(mktemp -d /tmp/nixos-installer.XXXXXXXX)
cp -- "$source_dir/flake.nix" "$source_dir/configuration.nix" "$source_dir/impermanence.nix" "$source_dir/desktop.nix" "$source_dir/style.nix" "$staging/"
if [[ -f $source_dir/flake.lock ]]; then cp -- "$source_dir/flake.lock" "$staging/"; fi
cat > "$staging/settings.nix" <<EOF
{
  hostname = "$hostname";
  username = "$username";
  timezone = "$timezone";
...
nix flake lock "path:$staging"

# BEGIN configuration preflight: no target disks have been changed yet.
# The target partitions do not exist yet. Use evaluation-only placeholders;
# the real hardware file is generated below after mounting the target.
cat > "$staging/hardware-configuration.nix" <<'EOF'
{ ... }: {
  fileSystems."/boot" = {
...
nix eval --raw "path:$staging#nixosConfigurations.nixos.config.system.build.toplevel.drvPath" >/dev/null \
  || die 'Configuration validation failed; no target disk changes have been made.'
rm -- "$staging/hardware-configuration.nix"
# END configuration preflight
...
cp -- "$staging/flake.nix" "$staging/flake.lock" "$staging/configuration.nix" "$staging/settings.nix" "$staging/impermanence.nix" "$staging/desktop.nix" "$staging/style.nix" /mnt/etc/nixos/
# Impermanence owns the config bind mount; omit it from the hardware scan.
umount /mnt/etc/nixos
nixos-generate-config --root /mnt --show-hardware-config > /mnt/persist/etc/nixos/hardware-configuration.nix
